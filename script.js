    ...
    shshofer.addEventListener('click',async function(){
      ...
      try{
        if(info){
          ...
          await batchGetValues(active_sheet, ['Sheet2!A2:B100'],muaji,"sh");
          calculate_Values();
          load_Sh_Or_G(shoferet,
            `<th> Shoferi </th>
            <th> Rr Naten</th>
    ...
    shgrop.addEventListener('click',async function(){
      ...
      try{
        if(info){
          ...
          alert("Gropa u shtua.");
          await batchGetValues(active_sheet, ['Sheet3!A2:B100'],muaji,"g");
          calculate_Values();
          load_Sh_Or_G(gropat,
            `<th> Gropa </th>
            <th> Rruget </th>`,"shgrop")
    ...
    shrrug.addEventListener('click',async function(){
      ...
      try{
        if(data!="jo" && nr!="jo" && ko!="jo" && sho!="jo" && gro!="jo" && shk!="jo"){
          ...
          await batchGetValues(active_sheet, ['Sheet5!A2:G1500'],muaji,"rr");
          load_Rruget()
          calculate_Values()
        }
          else{
    ...
    shshkarkim.addEventListener('click',async function(){
      ...
      try{
        if(info){
          ...
          await batchGetValues(active_sheet, ['Sheet7!A2:B100'],muaji,"sha");
          calculate_Values();
          load_Sh_Or_G(shkarkimet,
            `<th> Vend Shkarkimet </th>
            <th> Rruget </th>`,"shshkarkim");}
